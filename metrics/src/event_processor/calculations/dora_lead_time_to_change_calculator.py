...
class DORALeadTimeToChangeCalculator():
    ...
    def calculate_median_day_lead_time_for_deploy(self, push_events: [EventPush], deploy_event_date: str) -> int:
    ...
    def _calculate_lead_times(self, push_events: [EventPush], deploy_date: datetime) -> [int]:
        lead_times = []
        for event in push_events:
            push_date = self._parse_date(event.get_time(), "%Y-%m-%d %H:%M:%S")
