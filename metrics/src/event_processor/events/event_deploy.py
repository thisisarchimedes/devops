...
class EventDeploy(Event):
    ...
    def _report_deploy_frequency(self, deploy_frequency: float):
        event_dict = self._create_event_dict(EventTypes.CALC_DEPLOY_FREQUENCY, {"deploy_frequency": deploy_frequency})
    ...
    def _report_median_lead_time(self, median_lead_time: int):
        event_dict = self._create_event_dict(EventTypes.CALC_DEPLOY_LEAD_TIME, {"deploy_lead_time": median_lead_time})
        self._write_and_log_event(event_dict)
    ...
    def _calculate_median_lead_time(self, push_events: list[pd.DataFrame]) -> int:
        ...
        return calc.calculate_median_day_lead_time_for_deploy(push_events, self.get_time())
