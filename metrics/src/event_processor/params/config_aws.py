...
class ConfigAWS(Config):
    ...
    def get_db_table_name(self) -> str:
        ...
        if not db_table_name:
            raise ValueError(
                "DEVOPS_TABLE_NAME environment variable is not set.")
    ...
    def get_deployment_freq_timeframe_days(self) -> int:
        ...
        if not days:
            raise ValueError(
                "DEPLOYMENT_FREQ_TIMEFRAME_DAY environment variable is not set.")
    ...
    def _load_environment_variables(self):
        ...
        if not self.secret_store_name:
            raise ValueError(
                "SECRET_STORE_NAME environment variable is not set.")
    ...
    def _fetch_secret(self):
        try:
            response = self.secrets_manager_client.get_secret_value(
                SecretId=self.secret_store_name)
        except Exception as e:
            raise e
