...
class DORADeployFrequencyCalculator():
    ...
    def adjust_date_range(self, start_date: date, end_date: date) -> tuple:
    ...
    def adjust_to_previous_monday(self, some_date: date) -> datetime:
        """Adjust a date to the previous Monday."""

        return datetime.combine(some_date, time.min) - timedelta(days=some_date.weekday())

    def adjust_to_following_sunday(self, some_date: date) -> datetime:
    ...
    def create_weekly_date_range(self, start_date: date, end_date: date) -> pd.DatetimeIndex:
        """Create a weekly date range starting from Monday."""

        return pd.date_range(start=start_date, end=end_date, freq='W-MON')

    def calculate_weekly_deploy_counts(self, daily_deploy_volume: pd.DataFrame) -> pd.Series:
    ...
    def create_output_dataframe(self, date_range, weekly_deploy_counts: pd.Series) -> pd.DataFrame():
