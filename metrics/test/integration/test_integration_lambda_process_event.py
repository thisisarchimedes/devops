...
class TestIntegrationLambdaProcessEvent():
    ...
    def test_process_event_deploy(self, capfd):
        ...
        for line in out.split('\n'):
            assert not line.startswith('Error'), f"Test failed due to error print statement {out}"

        # Optionally, also check the standard error (stderr)
        for line in err.split('\n'):
            assert not line.startswith('Error'), f"Test failed due to error print statement {err}"
